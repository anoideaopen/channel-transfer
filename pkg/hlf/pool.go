...
func (pool *Pool) blockKeeper(key channelKey, provider hlfcontext.ChannelProvider) error {
	...
	pool.log.Debug("PFI1 ", checkPoint.Ver, " ", string(key))
	if err != nil {
	...
	pool.log.Debug("PFI2 ", checkPoint.Ver, " ", checkPointVersion, " ", string(key))
	...
	for pool.gCtx.Err() == nil {
		...
		case <-saver.C:
			go func() {
				pool.log.Debug("PFI3 ", checkPointVersion, " ", string(key))
